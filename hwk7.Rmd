...
#try new A
A <- cbind(M.t, matrix(rnorm(6*12),ncol=6))
det(A)
...
c. Don't know what's wrong here.
```{r}
h <- list(
...
```{r}
y.male <- male$spnbmd
g.im <- lapply(7:12, function(x) g.x(z = A.qr[,x], data = male$age))
...
qplot(male$age, y.male) + geom_line(aes(y=fit.gm, color="red")) + geom_line(aes(y=fit.hm, color="green"))


y.female <- female$spnbmd
g.i <- lapply(7:12, function(x) g.x(z = A.qr[,x], data = female$age))
...
qplot(female$age, y.female) + geom_line(aes(y=fit.gf, color="red")) + geom_line(aes(y=fit.hf, color="green"))
```